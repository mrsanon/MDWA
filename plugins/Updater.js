...
Amdi.operate({pattern: 'update$', fromMe: true, desc: Lang.UPDATER_DESC, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (commits.total === 0) {
        ...
        commits['all'].map(
            (commit) => {
                degisiklikler += '▷ [' + commit.date.substring(0, 10) + ']: ' + commit.message + ' ◁◁MR.TIMA▷▷\n';
            }
        );
        ...
        const buttonMessage = {
            ...
            footerText: 'DARK MAX © SPONSER AQUA BOT',
            buttons: buttons,
            headerType: 1
        }
...
var Action = ''
if (Build.LANG == 'SI') Action = '*👸🏻 Dark Max Updating...*'
if (Build.LANG == 'EN') Action = '*👸🏻 Dark Max Updating...*'
